...
begin
  i:=1;
  while r[i] = 'w' do inc(i);
  c:=r[i];
  for i:=i+1 to n do if (r[i] <> 'w') and (r[i] <> c) then exit;
  print(n);
end;
...
begin
  ...
  check;
  max:=0;
  next[n]:=1; for i:=1 to n-1 do next[i]:=i+1;
  pre[1]:=n; for i:=2 to n do pre[i]:=i-1;
end;
